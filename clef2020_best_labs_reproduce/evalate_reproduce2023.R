...
run_id <- paste0("2019_distributed_effort_", id)
#run_id <- paste0("2019_baseline_bm25_t1000")
...
run <- run %>%
  left_join(qrels_abs_test, by = c("topic_id" = "topic", "pid" = "pid")) %>%
  select(topic_id, interaction, pid, rank, score, run_id, relevant) %>%
...
run$relevance[which(is.na(run$relevance))] <- 0
...
eval_measures[, c("num_rels", "rels_found", "num_shown")] %>% 
  ...
  summarise(docs_shown = sum(num_shown), 
            avg_recall = mean(recall)) %>%
  print()

eval_measures %>%
  select(num_shown, recall_at_R, recall)
