...
run_id <- paste0("t", thresh)

# give run name
run_name <- paste0("2019_baseline_bm25_", run_id)
...
# topics <- read.table("../read_pubmed/pubmed_data/test_topics.txt", 
#                      header = T, 
...
for(topic in topics) {
  ...
  # subset qrels 
  #qrels <- qrels_abs_test[qrels_abs_test$topic == topic, c("pid", "relevant")]
  # subset qrels with pids only
  #qrels <- qrels[which(is.element(qrels$pid, documents)), ]
  ...
  # adjust if necessary
  if (k > thresh & thresh > 0) { # if k is greater than threshold
    k <- thresh
  }
  ...
  #     
  #     # keep track of the index "i" for ranking and selection strategy
  #     i <- i + 1
  #     
