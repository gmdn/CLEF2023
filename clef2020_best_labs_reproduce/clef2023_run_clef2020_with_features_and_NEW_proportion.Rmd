...
# if minus one, take all the terms in the relevant document
f  <- 0
# m2 angular coefficient after top k documents
m2 <- 1.0
# thresh: number of documents that a physician is willing to read per topic
thresh <- 600
...
# give run name
run_name <- paste0("2019_new_distributed_effort_", run_id)
...
docs_per_topic <- qrels_abs_test %>% 
  ...
  ungroup() %>% 
  arrange(n) %>%
  mutate(delta = thresh - n) %>%
  mutate(saved = if_else(delta < 0, 0, delta)) 

tot_saved <- sum(docs_per_topic$saved)

tot_docs_delta <- sum(docs_per_topic$n[docs_per_topic$saved == 0])

docs_per_topic <- docs_per_topic %>%
  mutate(show = if_else(saved > 0, n, thresh + n/tot_docs_delta * tot_saved))
...
for(topic in topics) {
  ...
  for(i in 1:k) {
    ...
    if(num_of_rel_documents > 0) {
      ...
      } else if (f == 0) { # increment f at each round
        features <- union(features,
                          head(order(p_q, decreasing = TRUE), i))
      } else if (f == -1) {
        
        features <- union(features,
                          head(order(p_q, decreasing = TRUE), 100))
  ...
  thr <- floor(thresh / 4)
  #### DIVIDE BY 4 HERE (or you will assess all the documents)
